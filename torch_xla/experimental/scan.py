...
def scan(
  ...
  Args:
    ...
    is_fn_pure: (Optional[bool]) If `fn` is pure, the tracing cache will be enabled. A pure function always produces the same output for the same input, and it doesn't have any side effects, meaning it doesn't modify any state outside of itself. Essentially, it's like a mathematical function that only depends on its input arguments.
...
def value_and_grad_partitioned(
  ...
  # The key is a tuple of the shapes, dtypes, and the pytree specs of the carry and xs.
